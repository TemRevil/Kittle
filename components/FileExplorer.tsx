...
const TreeNode: React.FC<{ 
  ...
  onFileClick: (path: string) => void;
  isLoadingFile?: string | null;
}> = ({ node, depth, activeFiles, onFileClick, isLoadingFile }) => {
  const [isOpen, setIsOpen] = useState(false);

  const isActive = activeFiles.some(f => f.name === node.path);
  const isLoading = isLoadingFile === node.path;
...
export const FileExplorer: React.FC<FileExplorerProps> = ({ nodes, activeFiles, onFileClick, isLoadingFile }) => {
  if (nodes.length === 0) {
    return (
